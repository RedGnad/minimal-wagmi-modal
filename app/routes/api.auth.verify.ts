// Resource route: POST /api/auth/verify -> { ok, token? }
// Vérification minimale SIWE côté serveur (doit être renforcée en production).

import type { ActionFunctionArgs } from "react-router";
import { consumeNonce } from "./api.auth.nonce";
...
const EXPECTED_DOMAIN = (import.meta as any).env?.VITE_SIWE_DOMAIN || "localhost:5173";

function extractNonce(message: string): string | null {
  // SIWE standard: line "Nonce: <value>"
...
function extractDomain(message: string): string | null {
  ...
  // Format: "<domain> wants you to sign in with your Ethereum account:" (SIWE)
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  // Récupération adresse via la signature
  try {
    ...
    return new Response(`Recover failed: ${e?.message || "error"}`, { status: 400 });
  }

  // Extraction du nonce et validation
  ...
  if (!consumed.ok) return new Response(consumed.error || "Nonce invalid", { status: 400 });
  ...
  // Génération d'un token de session éphémère (placeholder). En production: JWT signé ou cookie httpOnly.
